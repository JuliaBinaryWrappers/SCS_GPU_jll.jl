...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import SCS_GPU_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("SCS_GPU_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "SCS_GPU_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"SCS_GPU", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "SCS_GPU_jll", "src", "SCS_GPU_jll.jl"))
        @info("SCS_GPU_ll dev'ed out to /depot/dev/SCS_GPU_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("af6e375f-46ec-5fa0-b791-491b0dfa44a4"))
platforms = [Pkg.Artifacts.unpack_platform(e, "SCS_GPU", artifacts_toml) for e in artifacts["SCS_GPU"]]
...
if best_platform === nothing
    @debug("Unable to load SCS_GPU; unsupported platform $(triplet(platform_key_abi()))")
else
